...
encode_double_array = encode_double_array = (arg) ->
  build_array(arg).map((e) -> build_array(e).join(",")).join("|")

encode_key_value = encode_key_value = (arg) ->
  if _.isObject(arg)
...
build_eager = build_eager = (transformations) ->
  (for transformation in build_array(transformations)
    transformation = _.clone(transformation)
    _.filter([generate_transformation_string(transformation), transformation.format], present).join("/")
  ).join("|")
    
build_custom_headers = build_custom_headers = (headers) ->
  if !headers?
    return undefined
  else if _.isArray(headers) 
    ;
...
generate_transformation_string = generate_transformation_string = (options) ->
...
updateable_resource_params = updateable_resource_params = (options, params = {}) ->
